...
```json
{
  ...
  "steps": [
    {"stepNo": 1, "stepType": "tool_action", "name": "补充 task CLI warning、JSON 与 stderr 精确兼容测试"},
    {"stepNo": 2, "stepType": "tool_action", "name": "增加 CLI warning presenter 通道并保护 JSON 输出"},
    {"stepNo": 3, "stepType": "tool_action", "name": "恢复 task report/verify 前置校验 stderr 文本"},
    {"stepNo": 4, "stepType": "tool_action", "name": "验证 task CLI 专项测试、全量测试、lint 和 project doctor"}
  ]
}
```
