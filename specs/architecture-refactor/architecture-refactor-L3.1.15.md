...
```json
{
  ...
  "steps": [
    {"stepNo": 1, "stepType": "tool_action", "name": "补充 REMOVED 存活引用与组合操作回归测试"},
    {"stepNo": 2, "stepType": "tool_action", "name": "增加归档最终投影引用完整性校验"},
    {"stepNo": 3, "stepType": "tool_action", "name": "保持无引用删除与同批删除兼容"},
