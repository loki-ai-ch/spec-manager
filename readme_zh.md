...
做 UI、视觉或样式相关工作时，推荐把设计上下文放在 `specs/DESIGN.md`，作为 spec-manager 管理的 specs 体系的一部分。spec-manager 会把它当作可选的本地上下文，而不是 L2 技术设计的替代品。项目根目录 `DESIGN.md` 仍作为 legacy fallback 兼容。

- `spec-manager assist brief --request "<UI 需求>"` 会在需求命中设计相关意图且 `specs/DESIGN.md` 存在时自动带上 Design Context；若不存在则 fallback 到根目录 `DESIGN.md`。
- `spec-manager assist design-template` 默认写入 starter `specs/DESIGN.md`；默认不覆盖已有文件，除非传 `--force`。只有需要兼容旧根路径时才使用 `--out DESIGN.md`。
- `spec-manager assist design-export --format tokens-json` 会从默认 design context 导出规范化 tokens；可用 `--path DESIGN.md` 或 `--path specs/DESIGN.md` 强制指定文件。使用 `--format dtcg-json` 可输出当前 DESIGN.md schema 的 DTCG JSON 子集，`--format tailwind-json` 可输出 Tailwind v3 `theme.extend`，`--format tailwind-css` 可输出 Tailwind v4 `@theme` block，使用 `--out <file>` 可写入本地文件。
